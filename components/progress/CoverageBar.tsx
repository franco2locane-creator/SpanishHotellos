...
export default function CoverageBar({ label, completed, total, locked }: Props) {
  const pct = total > 0 ? Math.min(100, Math.round((completed / total) * 100)) : 0;
  return (
    <View style={styles.row} accessibilityLabel={`${label}: ${completed} of ${total}`}>
      <View style={styles.labelRow}>
        <Text style={styles.label} numberOfLines={1}>{label}</Text>
        <Text style={styles.frac}>{completed}/{total}{locked && completed === 0 ? ' 🔒' : ''}</Text>
      </View>
