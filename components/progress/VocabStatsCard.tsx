...
export default function VocabStatsCard({ learned, due, total, streak }: Props) {
  return (
    <View style={styles.card}>
      <Text style={styles.cardTitle}>Vocabulary</Text>
      <View style={styles.statsRow}>
        <Stat value={learned} label="Learned" color="#16A34A" />
        <Stat value={due} label="Due today" color={due > 0 ? Colors.gold : undefined} />
        <Stat value={total} label="Total cards" />
        <Stat value={streak} label={`Day streak${streak === 1 ? '' : 's'}`} color={streak > 0 ? Colors.gold : undefined} />
      </View>
    </View>
  );
}
